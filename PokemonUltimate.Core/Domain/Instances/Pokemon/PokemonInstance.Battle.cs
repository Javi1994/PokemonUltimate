...
namespace PokemonUltimate.Core.Domain.Instances.Pokemon
{
    ...
    public partial class PokemonInstance
    {
        ...
        public int GetEffectiveStat(Stat stat)
        {
            ...
            int stage = StatStageService.Default.GetStage(StatStages, stat);

            if (stat == Stat.Accuracy || stat == Stat.Evasion)
            {
                float multiplier = StatCalculatorService.GetAccuracyStageMultiplier(stage);
                return (int)(baseStat * multiplier);
            }

            int effective = StatCalculatorService.GetEffectiveStat(baseStat, stage);
        ...
        public int GetEffectiveStatRaw(Stat stat)
        {
            ...
            return StatCalculatorService.GetEffectiveStat(baseStat, stage);
        }
        ...
        {
            return StatStageService.Default.ModifyStage(StatStages, stat, change);
        }
        ...
        public void ResetBattleState()
        {
            ...
            StatStageService.Default.ResetStages(StatStages);
        }

...
0.  **Read Before Writing (MANDATORY)**:
    - **NEVER write code without reading existing code first**
    - **MANDATORY**: Read all existing code files related to the feature before implementing
    - **MANDATORY**: Check `code_location.md` to find where code lives
    - **MANDATORY**: Understand existing patterns, naming conventions, and structure
    - **MANDATORY**: Match existing code style and organization
...
1.  **Testability First (TDD Workflow)**:
    ...
    - **Test Structure Standard**:
      - **MANDATORY**: All tests MUST follow the structure defined in `docs/ai/testing_structure_definition.md`.
      ...
      - **See**: `docs/ai/testing_structure_definition.md` for complete structure and rules
...
- Every major system must have a corresponding Specification Document in `docs/features/[N]-[feature-name]/architecture.md` or sub-feature `architecture.md` (always use numbered format).
- Every major flow must have a Mermaid diagram.
- Roadmaps and implementation plans must be maintained in `docs/features/[N]-[feature-name]/roadmap.md` (always use numbered format).
- Workflow guides must be maintained in `docs/ai/workflow/`.
- Testing guides must be maintained in `docs/features/[N]-[feature-name]/testing.md` (always use numbered format). Shared testing strategy is in `docs/ai/testing_structure_definition.md`.
- **MANDATORY: Feature References in Code** - ALL code must reference its feature (see Section 5.6).
- **MANDATORY: Read Before Write** - Never write code without reading existing code first (see Section 5.7).
...
| Game Features Roadmap | `docs/features/5-game-features/roadmap.md` |
...
See `docs/ai/workflow/troubleshooting.md` for detailed problem-solving guide.
...
- **Project Structure**: See `docs/ai/GETTING_STARTED.md` for solution layout (Core/Content/Tests), namespaces, folder conventions, dependency flow.
...
- **Battle Mechanics Catalog**: See `docs/features/2-combat-system/architecture.md` Section 4 for comprehensive list of all BattleActions and IMoveEffects (40+ effects, 25+ actions) with implementation examples and priority ordering.
...
See `docs/features/2-combat-system/testing/integration_guide.md` for complete guide.

---

## 5. Feature Documentation Standards

### 5.1 Feature Naming Convention

**⚠️ CRITICAL: Always use numbered feature format**

- **Features**: `[N]-[kebab-case-name]` (e.g., `1-pokemon-data`, `2-combat-system`)
- **Sub-Features**: `[N.M]-[kebab-case-name]` (e.g., `2.5-combat-actions`, `3.1-pokemon-expansion`)
- **Display Names**: `Feature [N]: [Feature Name]` or `[N]: [Feature Name]`
- **Never use unnumbered paths**: Always include the number prefix

**Examples**:
- ✅ Correct: `docs/features/2-combat-system/`
- ❌ Wrong: `docs/features/combat-system/`
- ✅ Correct: "Feature 2: Combat System"
- ❌ Wrong: "Combat System" (without number)

### 5.2 Locating Feature Documentation

**Feature Documentation Location**: `docs/features/[N]-[feature-name]/`

**Required Documents** (all features):
...
**Process**:
1. **Determine Number**: Check `docs/features_master_list.md` for next available number
...
5. **Update Master List**: Add feature entry to `docs/features_master_list.md`
...
- Sub-Features (if applicable, numbered as `[N].[M]`)

**See**: `docs/feature_documentation_standard.md` Section "Creating New Features" for complete guide.

### 5.4 Creating New Sub-Features
...
5. **Update Master List**: Add sub-feature entry under parent feature
...
- Sub-Feature Name (Title Case)
- Folder (`[N.M]-[kebab-case-name]`)
- Description (brief)

**See**: `docs/feature_documentation_standard.md` Section "Creating New Sub-Features" for complete guide.

### 5.5 Feature Reference Standards

**In Documentation**: Always use numbered format (`Feature [N]: [Name]` or `[N]: [Name]`)  
**In File Paths**: Always use numbered format (`docs/features/[N]-[feature-name]/`)  
**In Code Comments**: Reference features with numbers  
**Master Reference**: `docs/features_master_list.md` - Single source of truth

**See**: `docs/feature_documentation_standard.md` for complete reference standards.

### 5.6 Feature Naming in Code

**⚠️ MANDATORY: ALL code must reference its feature**
...
**See**: [`feature_naming_in_code.md`](feature_naming_in_code.md) for complete guidelines and examples.

### 5.7 Code Reading Before Writing (MANDATORY)

**⚠️ CRITICAL: Never write code without reading existing code first**

**Before Writing ANY Code**:
1. Read `code_location.md` to find where code lives
2. Read all existing classes/interfaces in the feature's namespace
3. Understand existing patterns, naming conventions, and structure
4. Check how similar features are implemented
5. Match existing code style and organization

**Rule**: If you haven't read the existing code, you cannot write new code.

**Why**: Ensures consistency, prevents duplicates, maintains patterns, reduces refactoring needs.

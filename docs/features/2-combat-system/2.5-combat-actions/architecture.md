# Sub-Feature 2.5: Combat Actions - Architecture

> Complete technical specification of the BattleAction system and implementations.
...
The **Targeting System** determines **who** a move can hit. It bridges the gap between a Move's definition (`TargetScope`) and the dynamic Battle Field.

### 8.1 Core Definitions

`TargetScope` is defined in `MoveData`:
...
### 8.2 TargetResolver Service

*Namespace: `PokemonUltimate.Combat.Targeting`*

This service answers: *"I want to use Move X. Who can I click on?"*
...
public static class TargetResolver {
    ...
    public static List<BattleSlot> GetValidTargets(BattleSlot user, MoveData move, BattleField field) {
        ...
        // 3. Handle Redirection (Follow Me, Lightning Rod)
...
### 8.3 Integration with Actions

For multi-target moves, `UseMoveAction` expands the target list:
...
- **[Use Cases](use_cases.md)** - All action use cases (207 scenarios)
...
- **[Sub-Feature 2.2: Action Queue](../2.2-action-queue-system/)** - Actions are processed by queue
- **[Sub-Feature 2.4: Damage Calculation Pipeline](../2.4-damage-calculation-pipeline/architecture.md)** - Damage actions use pipeline
- **[Sub-Feature 2.8: End-of-Turn Effects](../2.8-end-of-turn-effects/architecture.md)** - Status actions integrate with status system

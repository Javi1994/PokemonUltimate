...
|-----|---------|----------|
| **Battle Debugger** | Battle statistics and analysis | Move usage stats, status effects, win/loss/draw rates |
| **Move Debugger** | Move testing and statistics | Damage stats, critical hits, status effects, action generation |
...
**Battle Debugger Tab**:
- Run multiple battles and analyze statistics
- Move usage statistics (most used moves per Pokemon)
- Status effect statistics (effects caused per Pokemon)
- Win/loss/draw rates
- Use Cases: Testing Pokemon matchups, verifying move patterns, analyzing status effects

**Move Debugger Tab**:
- Test moves multiple times and collect statistics
- Damage statistics (average, min, max, median)
- Critical hit rates, miss rates
- Status effect application rates (persistent and volatile)
- Action generation tracking
- Use Cases: Verifying move power ranges, testing status chances, analyzing move effects

**Type Matchup Tab**:
- Calculate type effectiveness combinations
- Single type vs single type or dual type
- Complete type chart table for any attacking type
- Effectiveness breakdown for dual-type defenders
- Use Cases: Verifying type chart, testing dual types, checking immunities

The unified debugger features:
...
- ✅ **Consistent experience** - Same look and feel across all debuggers
